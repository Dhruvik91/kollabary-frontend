...
export const adminService = {
    ...
    getVerifications: async (): Promise<VerificationRequest[]> => {
        const response = await httpService.get<VerificationRequest[]>(API_CONFIG.path.admin.verifications);
        return response.data;
    },
    ...
    processVerification: async (id: string, status: 'APPROVED' | 'REJECTED', notes?: string): Promise<VerificationRequest> => {
        const response = await httpService.patch<VerificationRequest>(`${API_CONFIG.path.admin.verifications}/${id}`, { status, notes });
        return response.data;
    },
