...
export const Footer = () => {
    ...
    const navLinks = [
        { name: 'About Us', href: FRONTEND_ROUTES.ABOUT },
        { name: 'Features', href: FRONTEND_ROUTES.FEATURES },
        { name: 'How It Works', href: FRONTEND_ROUTES.HOW_IT_WORKS },
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-12 mb-24">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <h4 className="font-bold text-[10px] uppercase tracking-[0.3em] text-primary/80">Navigation</h4>
                        <ul className="grid grid-cols-2 gap-2">
                            {navLinks.map((link) => (
                                <li key={link.name}>
                                    <Link href={link.href} prefetch={false} className="inline-block py-1.5 text-xs font-bold text-muted-foreground hover:text-primary transition-colors">
                                        {link.name}
                                    </Link>
                                </li>
                            ))}
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <h4 className="font-bold text-[10px] uppercase tracking-[0.3em] text-primary/80">Legal & Privacy</h4>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="space-y-4">
                            {contactDetails.map((detail) => (
                                <div key={detail.value} className="flex items-center gap-3 group cursor-default">
                                    <div className="w-9 h-9 rounded-xl glass-section border-primary/5 flex items-center justify-center text-muted-foreground group-hover:text-primary group-hover:border-primary/20 transition-all duration-500">
                                        <detail.icon size={16} />
                                    </div>
                                    <div>
                                        <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60 mb-0.5">{detail.label}</p>
                                        <p className="font-black text-xs tracking-tight group-hover:text-primary transition-colors">{detail.value}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
