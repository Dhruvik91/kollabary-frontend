...
function getCommonPinningStyles<T>(
  ...
  context: "header" | "body" | "footer" = "body"
): React.CSSProperties {
  ...
  // Header needs higher z-index than body to stay on top when scrolling vertically
  const baseZIndex = context === "header" ? 30 : 10;
  const zIndex = isPinned ? baseZIndex + 10 : baseZIndex;
  ...
  return {
    ...
    boxShadow: showLeftShadow
      ? "inset -4px 0 4px -4px oklch(0.65 0.15 15 / 0.2)"
      : showRightShadow
        ? "inset 4px 0 4px -4px oklch(0.65 0.15 15 / 0.2)"
        : undefined,
  };
...
export function DataTable<T>({
  ...
  const filteredData = useMemo(() => {
    if (!search || manualSearching) return data;
  ...
  const renderSummaryRow = () => {
    ...
    return (
      <tr
        className={`bg-muted/30 font-semibold text-foreground group border-t border-border glass-section ${stickyClass}`}
      >
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="rounded-xl border border-border bg-card/50 glass-card overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          ...
          >
            <div style={{ width: "100%" }}>
              <table className="w-full text-sm text-left text-foreground table-fixed border-collapse relative font-sans">
                <thead className="text-xs text-foreground uppercase bg-card/95 backdrop-blur-md sticky top-0 z-[50] border-b border-border font-display tracking-wider shadow-sm">
                  {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id} className="bg-muted/50 group">
                      {headerGroup.headers.map((header) => (
                        <th
                          key={header.id}
                          className={`py-3 px-4 bg-card group-hover:bg-muted/10 font-semibold text-left ${header.column.getCanSort()
                    ...
                    Array.from({ length: loadingRowCount }).map((_, index) => (
                      <tr key={index}>
                        {columns.map((_, colIndex) => (
                          ...
                          >
                            <div className="shimmer bg-muted h-full rounded-md"></div>
                          </td>
                        ))}
                      </tr>
                    ))
        ...
        {showPagination && !isLoading && filteredData.length > 0 && (
          <div className="flex flex-col gap-4 px-6 py-4 bg-background/30 border-t border-border backdrop-blur-sm">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full font-sans">
